...
result=$(sqlite3 /Users/$USERNAME/Library/Messages/chat.db 'SELECT text FROM message WHERE datetime(date/1000000000 + 978307200,"unixepoch","localtime") > datetime("now","localtime","-600 second") ORDER BY date DESC LIMIT 1;')

#You can append another `keyword` into the list to support other language Messages.
keyword=("验证码" "code", "code is:");
